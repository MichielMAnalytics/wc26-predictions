#!/usr/bin/env python3
"""
Predict the semi-finals from the ACTUAL bracket (ESPN), using the model retrained
through the Round of 16. Also picks the SF-phase top scorers (surviving 8 teams).
Writes data/predictions/sf_predictions.csv, sf_topscorers.csv, _sf_picks.json.
"""
...
# actual SF fixtures from ESPN (Jul 9-12, 'pre', real team names)
ej=json.load(open("data/results_2026/espn_ko3.json"))
sf=[]
for e in ej["events"]:
    if not ("2026-07-14" <= e["date"][:10] <= "2026-07-15"): continue
    ...
    sf.append((T.CODE2TEAM[h], T.CODE2TEAM[a], e["date"][:10]))
...
rows=[]
print("SEMI-FINALS — predicted scores (model retrained through R16):")
for a,b,dt in sf:
...
with open("data/predictions/sf_predictions.csv","w",newline="",encoding="utf-8") as f:
    w=csv.DictWriter(f,fieldnames=list(rows[0].keys())); w.writeheader(); w.writerows(rows)

# ---- SF top scorers: surviving 8 teams, ~1.5 expected SF+ matches per team via win prob ----
surviving=set()
for a,b,_ in sf: surviving.add(a); surviving.add(b)
...
# expected remaining matches ~ 1 (SF) + P(win SF)*... approx by win prob vs field
advp={}
for r in rows:
    advp[r["home"]]=r["p_home"]; advp[r["away"]]=r["p_away"]
def ematch(t): return 1 + 1.6*advp.get(t,0.3)   # SF + partial SF/final
...
for p in roster:
    ...
    ts.append({"player":p["player"],"team":p["team"],"position":p["position"],"exp_sf_goals":round(eg,2)})
ts.sort(key=lambda r:-r["exp_sf_goals"])
with open("data/predictions/sf_topscorers.csv","w",newline="",encoding="utf-8") as f:
    w=csv.DictWriter(f,fieldnames=list(ts[0].keys())); w.writeheader(); w.writerows(ts)
print("\nSF top scorers (pick 4):")
for i,r in enumerate(ts[:4],1): print(f"  {i}. {r['player']} ({r['team']}, {r['position']}) — {r['exp_sf_goals']}")
print("  next:", ", ".join(f"{r['player']} ({r['team']})" for r in ts[4:8]))

json.dump({"sf":[{"home":r["home"],"away":r["away"],"pred":r["pred"],"advances":r["advances"]} for r in rows],
           "top4":[{"player":r["player"],"team":r["team"],"pos":r["position"]} for r in ts[:4]]},
          open("data/predictions/_sf_picks.json","w"), ensure_ascii=False)
